...
#include "Constants.h"
#include "CamParams.h"   // [longtail] was arriving via LiDAR.h
#include <vector>
#include <string>
...
// [longtail] This used to be `getenv("DEEPGTAV_LOG_FILE")` alone, and log() was a
// no-op whenever it was unset. A Steam-launched game inherits Steam's environment,
// so it is ALWAYS unset in practice -- meaning every diagnostic the plugin writes,
// including the deadlock/timeout warnings, went nowhere. Default to a file in the
// game directory so the plugin can actually be debugged.
static std::string lt_logPath() {
	const char *env = getenv("DEEPGTAV_LOG_FILE");
	if (env != NULL && *env != '\0') return std::string(env);
	return std::string("DeepGTAV.log");   // cwd == the GTA V install dir
}

// TODO using such a static variable is not nice, but I don't see an easier solution right now
static auto lasttime = std::chrono::high_resolution_clock::now();

// [longtail] Two things were wrong with this for a run that lasts more than a
// few minutes:
//
//   1. ⚠ `fprintf(f, str.c_str())` passes the message as the FORMAT STRING. Any
//      '%' that ever reaches log() -- a path, a model name, a native name --
//      reads undefined varargs off the stack. That is a silent corruption or a
//      crash with no exception and no stack, which is exactly the failure shape
//      an unattended run reports. Use fputs: str is data, not a format.
//   2. The file grew without limit at ~230 KB per 20 s clip, because six lines
//      of per-frame trace were unconditional. An overnight batch wrote tens of
//      MB nobody reads, and paid an fopen/fclose per line to do it.
//
// ⚠ Do NOT "optimise" this into a persistent FILE*. These are `static` in a
// HEADER, so every translation unit gets its own copy -- a cached handle means
// four handles appending to one file with independent buffers. fopen/fclose per
// line keeps each line atomic; the cost is gone anyway now that the per-frame
// trace is off by default.
static long long lt_logBytes = 0;
static bool lt_logStopped = false;
static const long long LT_LOG_MAX_BYTES = 64LL * 1024 * 1024;

static void log(std::string str, bool override = false) {
    if (!(override || DEBUG_LOGGING)) return;
    if (lt_logStopped) return;

    FILE* f = fopen(lt_logPath().c_str(), "a");
    if (f == NULL) return;

    if (DEBUG_LOG_WITH_TIME) {
        auto thistime = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(thistime - lasttime);
        std::string prefix = "[+" + std::to_string(duration.count()) + " ms] ";
        fputs(prefix.c_str(), f);
        lt_logBytes += (long long)prefix.size();
        lasttime = thistime;
    }
    fputs(str.c_str(), f);      // NOT fprintf: str is data, not a format string
    fputc('\n', f);
    lt_logBytes += (long long)str.size() + 1;

    if (lt_logBytes > LT_LOG_MAX_BYTES) {
        fputs("[log] size cap reached in this module; further logging suppressed\n", f);
        lt_logStopped = true;
    }
    fclose(f);
}

// [longtail] Per-frame trace. Separated from log() so a long capture run is not
// paying six formatted lines and six file opens per frame for output nobody
// reads; flip DEBUG_LOG_PER_FRAME in Constants.h when the capture path itself is
// the suspect.
static void logFrame(std::string str) {
    if (DEBUG_LOG_PER_FRAME) log(str);
}

...
// [longtail] Per-frame trace (capture/send/JSON-build timings), routed through
// logFrame(). ~6 lines and a flush per captured frame -- ~230 KB per 20 s clip.
// Useful when the capture path itself is the suspect, dead weight otherwise.
const bool DEBUG_LOG_PER_FRAME = false;